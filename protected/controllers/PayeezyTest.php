...
require_once('PHPUnit/Util/Filesystem.php'); // workaround for PHPUnit <= 3.6.11
 require(dirname(__FILE__).'/Payeezy.php');
...
class PayeezyTest extends PHPUnit_Framework_TestCase
{
    ...
    public function setPrimaryTxPayload(){
        ...
        $card_holder_name = PayeezyTest::processInput("Darshan Hingu");
        $card_number = PayeezyTest::processInput("4788250000028291");
        $card_type = PayeezyTest::processInput("visa");
        $card_cvv = PayeezyTest::processInput("567");
        $card_expiry = PayeezyTest::processInput("1218");
        $amount = PayeezyTest::processInput("1200");
        $currency_code = PayeezyTest::processInput("USD");
        $merchant_ref = PayeezyTest::processInput("Astonishing-Sale");
    ...
    public function setSecondaryTxPayload($transaction_id, $transaction_tag, $amount, $split_shipment){
        ...
        $transaction_id = PayeezyTest::processInput($transaction_id);
        $transaction_tag = PayeezyTest::processInput($transaction_tag);
        $amount = PayeezyTest::processInput($amount);
        $currency_code = PayeezyTest::processInput("USD");
        $merchant_ref = PayeezyTest::processInput("Astonishing-Sale");
        $split_shipment = PayeezyTest::processInput($split_shipment);
    ...
    {
        $primaryTxResponse_JSON = json_decode(self::$payeezy->authorize(PayeezyTest::setPrimaryTxPayload()));
        PayeezyTest::assertEquals($primaryTxResponse_JSON->transaction_status,"approved");
    }

    public function testPurchase()
    {
        $primaryTxResponse_JSON = json_decode(self::$payeezy->purchase(PayeezyTest::setPrimaryTxPayload()));
        PayeezyTest::assertEquals($primaryTxResponse_JSON->transaction_status,"approved");
    }
    ...
    public function testCapture()
    {
        ...
        $secondaryTxPayload = PayeezyTest::setSecondaryTxPayload($primaryTxResponse_JSON->transaction_id
                                                            ,$primaryTxResponse_JSON->transaction_tag
                                                            ,$primaryTxesponse_JSON->amount
                                                            ,null);
        ...
        PayeezyTest::assertEquals($secondaryTxResponse_JSON->transaction_status,"approved");
    }
